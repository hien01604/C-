...
int exponentialSearch(int arr[], int size, int target) {
    if (arr[0] == target) return 0; // Nếu phần tử đầu tiên khớp
    ...
    // Tìm kiếm nhị phân trong khoảng [bound / 2, min(bound, size - 1)]
...
int main() {
    ...
    cout << "Exponential Search for 10: " << exponentialSearch(arr, size, 10) << endl;
